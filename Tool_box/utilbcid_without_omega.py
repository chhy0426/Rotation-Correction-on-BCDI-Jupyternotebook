...
def com(frame):
    """
    Calculate the center of mass for a 2D grid of mass values.

    Args:
    mass_grid (np.array): A 2D numpy array where each element represents the mass at that position.

    Returns:
    tuple: A tuple (x_center, y_center) representing the center of mass coordinates.
    """
    total_mass = np.sum(frame)
    if total_mass == 0:
        return None  # Avoid division by zero if the total mass is zero.

    y_indices, x_indices = np.indices(frame.shape)

    x_center = np.sum(x_indices * frame) / total_mass
    y_center = np.sum(y_indices * frame) / total_mass
...
class ProgressPlot(object):
    ...
    def update(self, W, Pjk, errors, vmax=0.1):
        ...
        ax[2].imshow(np.abs(Pjk), vmax=np.abs(Pjk).max(), aspect='auto')
        ax[3].plot(errors)
